...
function searchButtonFunc(){
  ...
  const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(DELETE_SHEET_NAME);
  ...
  try{
    ...
    if(shapedTweets.length){
      ...
      Browser.msgBox("条件を満たすツイートが存在しませんでした。", Browser.Buttons.OK); 
    }
  }catch(e){
...
function getTweetIdByTime(targetDate, maxid=0){
  ...
  // 指定時刻以前のツイートを検索
  const found = tweetJsons.find(tweetJson => new Date(Date.parse(tweetJson.created_at)) <= targetDate);
  
  if(found === undefined){
    // 存在しないなら次の200件を検索する
    const lastTweetJson = tweetJsons.slice(-1)[0];
    
    console.log('ID:%s DATE:%s', lastTweetJson.id_str, lastTweetJson.created_at);
