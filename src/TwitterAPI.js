...
function getTweetJsons(maxid = 0) {
  ...
  let apiurl =
    "https://api.twitter.com/1.1/statuses/user_timeline.json?count=200&trim_user=t&include_rts=1";
  if (maxid != 0) {
    apiurl += "&max_id=" + maxid;
  }
...
function getLastTweetURL() {
  ...
  const response = twitterService.fetch(
    "https://api.twitter.com/1.1/statuses/user_timeline.json?count=1&trim_user=t",
    twMethod
  );
